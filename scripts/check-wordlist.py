...
NUMS = "①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳㉑㉒㉓㉔㉕㉖㉗㉘㉙㉚㉛㉜㉝㉞㉟㊱㊲㊳㊴㊵㊶㊷㊸㊹㊺㊻㊼㊽㊾㊿❶❷❸❹❺❻❼❽❾❿⓫⓬⓭⓮⓯⓰⓱⓲⓳⓴⓵⓶⓷⓸⓹⓺⓻⓼⓽⓾"  # ①-㊿ 五十个，再接 ❶-❿ ⓫-⓴ ⓵-⓾ 三组反白圈码到 80
# 等式行的左边也要放行带重音的拉丁字母。**2026-09-06 之前它只认 ASCII**，
# 于是「crème caramel = 焦糖蛋奶布丁」「à la carte = 点菜」「résumé = 简历」
# 这类行根本不算等式 —— 全表 51 行。后果有两层：等式密度被低算（那是
# check-wordlist 自己的闸门），以及 audit-padding 判重复义项时
# first_eq 返回 None、整条比较被跳过。caramel ⑤⑥ 两个义项挂着一模一样的
# 「crème caramel = 焦糖蛋奶布丁」，闸门一声没吭，是回填时用眼睛看出来的。
# 下面 WORD_OK 的注释早就写着「词条首行允许带重音的拉丁字母」，
# 可 EQ 一直没跟着改 —— 同一件事在两个正则里只做对了一半。
EQ = re.compile(r"^([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ0-9’'\-\s.]{0,44}?)\s*=\s*(.+)$")
